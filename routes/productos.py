...
def get_empresa_id():
    if current_user.rol == 'god':
        return request.args.get('empresa_id', type=int)
...
def listar():
    ...
    return jsonify([p.to_dict() for p in q.order_by(Producto.categoria, Producto.nombre).all()])
...
def crear():
    ...
    if Producto.query.filter_by(codigo=data['codigo'], empresa_id=empresa_id).first():
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            db.session.add(Producto(
                codigo=codigo, nombre=str(nombre).strip(),
...
def plantilla():
    ...
    for row, data in enumerate([('PRD-001', 'Ejemplo Producto 1', 'Categoría A', 100, 10),
                                 ('PRD-002', 'Ejemplo Producto 2', 'Categoría B', 50, 5)], 2):
